...
export const mapFieldsPagSeguroCard = (p: TPagSeguroCard,
    ...
    p.customer.phones[0].country = person?.address.zipCode?.city?.country.ddi.toString() || "55" as any
    ...
    p.shipping.address.city = person?.address.zipCode?.city?.name.toString() || "Barbosa Ferraz" as any
    p.shipping.address.region_code = person?.address.zipCode?.city?.state.acronym.toString() as any
    p.shipping.address.country = person?.address.zipCode?.city?.country.acronym.toString() || "BRA" as any
