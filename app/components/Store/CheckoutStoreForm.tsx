...
export default function CheckoutStorePage({
  ...
  const itemImages = responseImages.filter((img) => img.idItem === item.id);
    ...
    <>
      <div className="w-full max-w-6xl mx-auto px-4 py-4">
        ...
        >
          ...
          <span className="text-blue-400">{item.subGroup.group.name}</span>
          <span className="text-gray-400">›</span>
          <span className="text-blue-400">{item.subGroup.name}</span>
        </nav>
      </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="w-full md:w-1/2 flex flex-col lg:flex-row items-center justify-center gap-4">
                  {/* Imagem principal */}
                  <div className="w-full flex-1 flex items-center justify-center order-1">
                    {itemImages.length > 0 ? (
                      <img
                        src={`${process.env.NEXT_PUBLIC_API_IMG}/${item.id}/${imageSelected || itemImages[0].fileName}`}
                        alt={item.imagem || item.name}
                        className="max-w-67 p-1 object-contain rounded-lgp-2"
                      />
                    ) : (
                  ...
                  {/* Miniaturas */}
                  <div className=" max-w-90 lg:w-auto grid grid-cols-4 sm:grid-cols-5 lg:grid-cols-1 gap-1 justify-items-center order-2 lg:order-1 max-h-500 overflow-y-auto p-1">
                    {itemImages.length > 0 ? (
                      itemImages.map((img) => (
                        <button
                          key={img.id}
                          ...
                          className="cursor-pointer w-16 h-16 sm:w-18 sm:h-18 p-1 rounded-md border border-gray-300 hover:border-blue-500 transition flex items-center justify-center bg-white"
                        >
                          <img
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 h-fit">
              <h2 className="text-lg font-semibold mb-5">Resumo da compra</h2>
              ...
              {/* BOTÃO COMPRAR */}
              <button
                className="cursor-pointer w-full mt-6 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-md transition"
                onClick={() => buyNow()}
              >
                {" "}
                Comprar agora{" "}
              ...
              {/* CARRINHO */}
              <button
                className="cursor-pointer w-full mt-3 bg-blue-100 hover:bg-blue-200 text-blue-700 font-semibold py-3 rounded-md transition"
                onClick={() => addToCart()}
              >
