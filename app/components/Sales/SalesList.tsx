...
export default function SalesList({ sales, cancelSale }: Props) {
            ...
            <table className={globalStyles_table_list}>
                {sales.length > 0 && <thead className={globalStyles_thead_list}>
                    <tr>
                        ...
                        {isAdmin && <th className={`${globalStyles_th} text-center bg-gray-600`}><FreeCancellationIcon titleAccess='Cancela Nota'/></th>}
                ...
                <tbody className={globalStyles_tbody_list}>
                    {sales.map((sale: TSaleResponse) => (
                        <tr key={sale.id} className={globalStyles_tr}>
                            ...
                            {isAdmin && <td className={`${globalStyles_td} text-center`}>
                                <button
                                    type="button"
                                    onClick={() => {
                                        if (!sale.cancel) {
